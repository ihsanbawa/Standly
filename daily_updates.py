...
async def fetch_tasks_from_todoist(todoist_token, filter):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if response.status == 200:
            ...
            return [(task['content'], task.get('due', {}).get('date')) for task in tasks]
        else:
            print(f"Failed to fetch tasks with filter '{filter}', status code: {response.status}")
            return []
...
async def post_daily_update(bot, user_id, database):
    ...
    channel = discord.utils.get(guild.text_channels, name=user_info['monitored_channel_name'])
    if not channel:
...
async def get_or_create_thread(channel, thread_name):
    thread = discord.utils.find(lambda t: t.name == thread_name and isinstance(t, discord.Thread), channel.threads)
    if not thread:
...
async def fetch_completed_tasks_from_todoist(todoist_token):
    ...
    since_date = yesterday.strftime('%Y-%m-%dT00:00:00Z')  # Start of yesterday in UTC
    print("Yesterday for completed tasks", yesterday)
    data = {
        "since": since_date
    }
